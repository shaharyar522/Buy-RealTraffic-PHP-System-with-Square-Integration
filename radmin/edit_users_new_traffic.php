...
{ $action='edit'; }
   ...
   $unassigned_login_credits_query = "SELECT * FROM unassigned_traffic_credits WHERE user_id = '".$term."'";
...
$unassigned_login_credits_query = "update unassigned_traffic_credits set login_credits = {$frm['login_credits']} WHERE user_id = '".$term."'";
   $unassigned_login_credits_result = mysqli_query($GLOBALS["___mysqli_ston"], $unassigned_login_credits_query) or die(mysqli_error($GLOBALS["___mysqli_ston"]).$unassigned_login_credits_query);
...
function login_site_balance($sid){
	...
	$sql_ssv = "SELECT * FROM traffic_sites WHERE user_id = '$term' order by sid ASC";
	$cntsas = '0';
...
$page_content = "<div align=center><b>#User Traffic Sites</b><br><br></div>";
