...
echo "tb_site_setting"; die;
   ...
   /*
    a)  Check For Expired Traffic Units.
    
    Select where $unit_status = Active and $unit_paid = No Compare the current date to the
    $purchase_date of these entries.  If the current date is greater than or equal to
    $days_unit_lasts from the $purchase_date, then set $unit_status = Expired.
    
     - $unit_expire_date = Date Traffic Units Expire.
       This is computed as $days_unit_lasts (which will be set at 40 days) from the $purchase_date
       In other words, ($purchase_date + $days_unit_lasts).

       So if a member purchase 5 Traffic Units on 2015-01-22, those 5 Traffic Units will expire
       40 days from the $purchase_date.

       So those 5 Traffic Units will expire on 2015-03-02
       This date will be displayed on the Member's Pay Stats table on the main page of
       the member's area and is updated each time a member purchases
       a Traffic Unit.  Note:  $days_unit_lasts field is in the Site Settings table.
   */
       
       fwrite($fp,"Today : $today \n");

      /*                        
         purchased date :  2015-01-22
         expired date   :  2015-03-02  
         cron will expired the units on 2015-03-03 12:05 AM
      */ 

      //Check For Expired Traffic Units.
      //$upx_traffic_units = "UPDATE   $tb_traffic_unit_purchase_history    SET   unit_status  = 'Expired',    cron_expire_date = '$today'    WHERE  unit_status = 'Active'     AND unit_paid   = 'No'    AND     unit_expire_date <= '$today' ";
      
      $upx_traffic_units = "UPDATE   $tb_users    SET   cb_page_status  = 'Disabled',    cb_expire_date = '$today'    WHERE  cb_page_status = 'Active'     AND      cb_expire_date <= '$today' ";
      ...
      #############################################################################################################
   /*
    b) Update "url_users table and Member's Pay Stats table.
    
    -  Update the number of $expired_units a member currently has.
       This is calculated as follows:  select member's entries where $unit_status = Expired
       and where $unit_paid = No and then add the $quantity for those entries and update the
       url_users table and display this value on the Member's Pay Stats table on the main page 
       of the member's area.
       
    -  Update the number of $active_units a member currently has.
       To compute $active_units (the number of Active traffic units a member has),  
       select where $unit_status = Active and where $unit_paid = No.
       and add the $quantity of these entries and update the url_users table and
       display this value on the Member's Pay Stats table on the main page of the member's area.
   */           
       ...
       if($num_users > 0){
          ...
          while($row_users = mysqli_fetch_array($res_users)){
                ...
                */                
                ...
                /*
                - Update the Total Cash Back Due = ($total_cash_back_due)  
                  
                  This is calculated as follows:  
                  
                    select  where $unit_status = Expired and where $unit_paid = No add the $amount for 
                    those entries x $cash_back_rate
                  
                    update the url_users table and display this value on the Member's Pay Stats table 
                    on the main page of the member's area.
                */                
...
EOD;
      ...
      ####################################################################################################
        ...
        /*
        c)  Have the system send me an email of Members whose Traffic Units expired that day.
            - The Subject should read:  "Members Traffic Units Expiration"
              The body of the email should include the following items:
              Below are members whose Traffic Units have expired today.
            - User ID#  Quantity  Package Name  Amount  Purchase Date  Unit Expiration Date
        */
