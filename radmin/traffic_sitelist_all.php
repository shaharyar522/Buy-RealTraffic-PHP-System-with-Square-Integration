...
include("$CONFIG->templatedir/header.php");
 ...
 $orderbytext = " ORDER BY  new_time  DESC";
   if(isset($_REQUEST['status']) && $_REQUEST['status'] == 'W') { $statsql = "AND status = 'W'";  }
   if(isset($_REQUEST['status']) && $_REQUEST['status'] == 'L') { $statsql = "AND status = 'L'";  }
   if(isset($_REQUEST['status']) && $_REQUEST['status'] == 'H') { $statsql = "AND status = 'H'";  }
   if(isset($_REQUEST['status']) && $_REQUEST['status'] == '1') { $statsql = "AND status = 'A'";  }
 $sql_list = "select * from traffic_sites where 1 $statsql  $orderbytext  ";

//if($status =='N'){
// $sql_list = "select * from traffic_sites where 1  and status = 'N'";
//    }
...
$page_content = "<div class=text id=heading  align=center><h3> Traffic List All Info </h3></div>";


$page_content .= "<form name=form1 method=post action=traffic_sitelist_all.php?act=search>
...
if ($status == 'N'){
 ...
 $page_content .= "<tr class=tblheader>
    <td align=center colspan=2 >
       <span class=text><a href='traffic_sitelist_all.php?act=search&status=A'>ALL </a></span>|
       <span class=text><a href='traffic_sitelist_all.php?act=search&status=L'>ACTIVE</a></span>&nbsp;|&nbsp;
       <span class=text><a href='traffic_sitelist_all.php?act=search&status=H'>PAUSE</a></span>&nbsp;|&nbsp;
       <span class=text><a href='traffic_sitelist_all.php?act=search&status=1'>SUSPEND</a></span>&nbsp;|&nbsp;
       <span class=text><a href='traffic_sitelist_all.php?act=search&status=W'>WAITING</a></span>
    </td>
  </tr>";
...
if ($status == 'W'){
  ...
  $page_content .= " <tr class=tblheader>
    ...
    <td align=center colspan=2 class=text>
       ...
       <span class=text><a href='traffic_sitelist_all.php?act=search&status=L'>ACTIVE</span>&nbsp;|&nbsp;
...
while($row_zz  = mysqli_fetch_array($res_list)){
 ...
 $page_content .= "</td>
       ...
       <td align=left width=30% class=text><a href='traffic_site_info_approve.php?sd={$row_zz['sid']}'>";
...
/*
if($status =='N'){
    $sql = "SELECT COUNT(sid) as login_sites FROM traffic_sites WHERE status='W'";
    $res = mysql_query($sql);
