...
<table  border=\"0\" class=text>
  ...
  <tr>
    <td colspan=\"2\">
	<input type=button class=button value=\"Add Text ad \" onclick=\"validare();\"></td>
  </tr>
</table></form>";
}//if
else
{$ce=$_POST['ce'];
$s=db_query("insert into textads(username,title,text,url,urlto) values('".$_SESSION['ADMIN']["admin"]["username"]."','".$_POST['btitle']."','".addslashes(htmlspecialchars($_POST['btext']))."','".$_POST['burl']."' ,'".$_POST['btourl']."')");
