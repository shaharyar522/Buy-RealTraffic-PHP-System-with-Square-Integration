...
if ( $_POST["g-recaptcha-response"] != ''  ) {
	...
	//	$msg .= "<li>Please Enter Your Venmo Username.  Your Referrals Will Use This Username To Pay You Your Sponsor Fee.</li>";
                ...
                // from google console
                $secret_key = '6Lch6wgpAAAAAPwbbCLju_vXzwtR4Jx6tVBbfhFs';
                ...
                if ($response->success == true) {
                   // echo '<script>alert("Google reCAPTACHA verified")</script>';
                    	...
                    	$err=1;
		                $msg .= "<li><font color=red><b>Please Put A Check Mark In The Recaptcha Box</b></font></li>";
...
//if (empty($frm["venmo_username"])) {
	...
	if (isset($frm["username"]) && ((strlen($frm["username"])<3)||(strlen($frm["username"])>15))){
		$err=1;
		$msg .= "<li>The username must have minimum 3 characters and maximum 15</li>";

	}if (isset($frm["username"]) && username_exists($frm["username"])) {
	$err=1;
...
$err=1;
		...
		$err=1;
		$msg .= "<li>Please Check The Checkbox To Agree To Our Term Of Service</li>";
	...
	$err=1;
	 ...
	 if (isset($frm["email"]) && email_exists($frm["email"])) {
		$err=1;
		$msg .= "<li>The email address <b>" . $frm["email"] ."</b> already exists on our database</li>";

	}if (isset($frm["email"]) && !validate_email($frm["email"])) {
$err=1;
		$msg .= "<li>Invalid email address( ".$frm["email"].") </li>";

	}
	
	if (isset($frm["password"]) && $frm["password"]!=$frm["rpassword"])
	 {$err=1;
		
		$msg .= "<li>The passwords do not match.</li>";

	} 
	 if (isset($frm["password"]) && ((strlen($frm["password"])<5)||(strlen($frm["password"])>15))){
		$err=1;
...
$_SESSION["account"]="true";
	...
	`lastname` , `sponsor` , `paypal` , `stormpay` , `egold` , `libertyreserve` , `assuredpay`, `solidtrustpay`,
	 ...
	 `joindate` , `last_ip`, `membership_status` ,cb_nickname , cb_page_status,cb_page_hits,venmo_username) VALUES (
...
'".htmlspecialchars($frm["firstname"])."', '{$frm['email']}',
 ...
 now(), '{$REMOTE_ADDR}'       ,'Free Member' , '{$frm['nickname']}','Disabled',0,'{$frm['venmo_username']}' )");
 //chances
...
</b><br><br>In order to get started, click on the Login link at the top of this page
and then log into your account.  Next, click on the \"Paid Services\" link to purchase any of our services.
...
";
		...
		$subject2 = $_SESSION["sponsor"]["firstname"].", Congratulations You Just Referred A New Member To Buy-RealTraffic.com";
		$message2 ="Hi  ";
		$message2 .=$_SESSION["sponsor"]["firstname"];
$message2 .=" ,<br><br>
Mike Borders here.  Just a short note to let you know you just referred
<br>
a new member to Buy-RealTraffic.com. If this member decides to Upgrade
<br>
...
Admin
<br>
<a href=http://www.buy-realtraffic.com>Click Here To Log Into Your Account</a>";
...
$_SESSION["account"]="true";
	...
	`lastname` , `sponsor` , `paypal` , `stormpay` , `egold` , `libertyreserve` , `assuredpay`, `solidtrustpay`,
	 `fleetpay`, `moneybookers`, `dollardeliverys`,
	 `joindate` , `last_ip`, `membership_status` ,cb_nickname , cb_page_status,cb_page_hits) VALUES (
'".trim(htmlspecialchars($frm["username"]))."', '$frm['password']', 
'".htmlspecialchars($frm["firstname"])."', '$frm['email']',
 '".htmlspecialchars($frm["lastname"])."', '$found_sponsor', '$frm['paypal']', '$frm['stormpay']', 
 '$frm['egold']', '$frm['libertyreserve']',  '$frm['assuredpay']','$frm['solidtrustpay']','$frm['fleetpay']','$frm['moneybookers']','$frm['dollardeliverys']',
 now(), '$REMOTE_ADDR'       ,'Free Member' , '$frm['nickname']','Disabled',0 )");
 //chances
 $nrb=db_fetch_row(db_query("select max(finish) from chances"));
$nr=$nrb[0];
$nri=$nr+1;
...
and then log into your account.  Next, click on the \"Paid Services\" to purchase any of our services.
<br><br>Your details have been sent to <b>".$frm["email"]."</td></tr></table></td></tr></table></b><br> <br><br>";
//mail the user
session_register("account");
	...
	$s=mysql_query("select wcm from administration where id=1") or die(mysql_error());
	while($r=mysql_fetch_array($s)){$message=$r[0];}
	$message=str_replace("%username%",$frm["username"],$message);
	$message=str_replace("%firstname%",$frm["firstname"],$message);
	$message=str_replace("%lastname%",$frm["lastname"],$message);
	$message=str_replace("%email%",$frm["email"],$message);
