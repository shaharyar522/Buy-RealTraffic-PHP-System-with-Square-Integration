...
function traffic_site_balance($sid){
    ...
    return $row_count_info[0];
	 ...
	 //$sql_xyz = "SELECT * FROM unassigned_traffic_credits WHERE user_id =".$_SESSION["user"]["id"];
	  $sql_xyz = "SELECT * FROM unassigned_traffic_credits WHERE user_id ='".$_SESSION["user"]["username"]."'";
 	 ...
 	 $row_xyz = mysqli_fetch_array($res_xyz); 
         $user_balance=$row_xyz['traffic_credits'];
         $loggin_user_points = $row_xyz['traffic_credits'];
  ...
  $cnts = '0';
  $curr_tab_pts_xxx='0';
// $sql_vv = "SELECT * FROM traffic_sites WHERE user_id = ".$_SESSION["user"]["id"]." order by sid ASC";  
   $sql_vv = "SELECT * FROM traffic_sites WHERE user_id = '".$_SESSION["user"]["username"]."' order by sid ASC"; 
  ...
  while($row_vv = mysqli_fetch_array($res_vv)) {
          $site_points   = traffic_site_balance($row_vv[0]);
...
if($_REQUEST['division'] == '1'){
       ...
       for($v=0;$v<$number_of_ids;$v++){
           ...
           $curr_ac_pts  = (int) $row_xyz['traffic_credits'];
...
///////////////////////////////----- VALIDATION BEGINS-----------///////////////////////
           ...
           if($curr_tab_pts == '0'){ 
               if($curr_ac_pts < $curr_frm_pts){
                  $err_msg_pt = "<li> You only have $curr_ac_pts Unassigned Traffic credits!</li>";
               }//
               //if($curr_ac_pts >= $curr_frm_pts){
               if($curr_ac_pts >= $curr_frm_pts){
   ...
   if($err_msg_pt == ""){ 
      ...
      #$My_Pending_Points = ($loggin_user_points - $curr_frm_pts);
  	  //  $sql_req = "UPDATE unassigned_traffic_credits SET traffic_credits = '$Pending' WHERE user_id =".$_SESSION["user"]["id"]; 
  	    $sql_req = "UPDATE unassigned_traffic_credits SET traffic_credits = '$Pending' WHERE user_id ='".$_SESSION["user"]["username"]."'"; 
       ...
       //After Error
        ...
        for($w=0;$w<$number_of_ids;$w++){
               $ups_del = "UPDATE traffic_clicks set points = '$spointx[$w]' WHERE site_id = '$sidx[$w]' limit 1"; 
    ...
    //  update_textAdds_status();
     ...
     echo "<META HTTP-EQUIV='Refresh' CONTENT='0; URL=traffic_sites.php?error_msg=$err_msg'>";
     exit();
   }
}
                  ...
                  <blockquote>
                   ...
                   You currently have<font color=red> ';
                    if(  isset($user_balance)  ){
                     ...
                     $page_content .='</font color> Unassigned traffic credits. Please enter the number of visitors you want each site to receive and click the Submit button below. NOTE: When entering the number of visitors, please do not use commas.
            ...
            <td width="100%">
               <div align="center">
                  ...
                  <table border="0"  cellpadding="3" cellspacing="0" width="100%"  style=" border: 1px solid #808080;font-size: 14px;" tr bgcolor="#1B476E" height="30">  
                    <tr><td align=center><font color=#ffffff>Assign Traffic Credits</font></td></tr>
                </table>   
                ...
                <tr>
                    <td width="100%"><form name="assx" method="post" action="traffic_site_share.php#ASSIGN">
                       <div align="center">
...
//$sql_zz = "SELECT * FROM traffic_sites WHERE user_id = ".$_SESSION["user"]["id"]." order by sid ASC"; 
//  $sql_zz = "SELECT * FROM traffic_sites WHERE user_id=55  order by sid ASC";	
    $sql_zz = "SELECT * FROM traffic_sites WHERE user_id='" .$_SESSION["user"]["username"]. "'  order by sid ASC";
	...
	while($row_zz = mysqli_fetch_array($res_zz)) { 
         ...
         // $package_list[$cnt]['site_balance'] = traffic_site_balance($row_zz[0]);
         // $package_list[$cnt]['site_meet']    = login_site_meet($row_zz[0]);
          $cnt++;
                    ...
                    //   $page_content .= traffic_site_balance($row_zz["sid"]) ;  
                    $bbalance    =traffic_site_balance($row_zz["sid"]);    
	...
	<tr><td colspan=6 align=center ><b><font color=blue><a href=\'traffic_sites.php\' title=\'BACK\'><font color="blue">BACK</font></a></font></b></td></tr>
	';
        ...
        if($resultq > 0){
         ...
         $page_content .=' <tr>
                  <td width="100%" colspan=6><p align="center"><input type=image src="../images/submit55.png"> &nbsp;&nbsp;<img src="../images/cancel55.png"  onclick="javascript:window.location=\'traffic_sites.php\'">
                  </p>
                  </td>
             </tr>
