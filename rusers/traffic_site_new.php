...
include'../config.php';
require_login();
$title="$CONFIG->sitename Traffic Sites New";
include("$CONFIG->templatedir/header.php");
 ...
 $sql_site = "select * from url_site_setting";
  ...
  $traffic_sites_allowed = $rec_site["traffic_sites_allowed"];
  ...
  $global_user_avl_site        = $rec_site['number_of_site'];  
    ...
    if (isset($_REQUEST['act']) && $_REQUEST['act'] == "newsite") {
  	// $sql_cc = "SELECT * FROM traffic_sites WHERE user_id = '$_SESSION['user_id']' AND status != 'D'";
  $sql_cc = "SELECT * FROM traffic_sites WHERE user_id = '".$_SESSION["user"]["username"]."' AND status != 'D'";
 	 ...
 	 $res_cc = mysqli_query($GLOBALS["___mysqli_ston"], $sql_cc)or die(mysqli_error($GLOBALS["___mysqli_ston"]).$sql_cc);
            ...
            if($num_cc >= $traffic_sites_allowed){
		   	   $error_msg .= "<li>Sorry, you can only add up to $traffic_sites_allowed sites!</li>";
            }
  ...
  if(  isset($site_url) && $site_url!=null && isset($site_name) && $site_name!=null && $error_msg==null  ) {
			    $sql = "INSERT INTO traffic_sites set
                                                site_name  = '$site_name',
                                                site_url   = '$site_url',
                ...
                $sql_ggg = "INSERT INTO traffic_clicks SET
                                                       user_id  = '".$_SESSION["user"]["username"]."',
                                                       site_id  = '$mid',
                                                       points   = '0'  ";
				...
				$error_msg  = "<li> New traffic site added successfully</li>";
				#$site_name  = "";
				#$site_url   = "";
                echo "<META HTTP-EQUIV='Refresh' CONTENT='0; URL=traffic_sites.php?error_msg=$error_msg'>";
                exit();
           }//EndInsert
...
<td colspan="3" align="center" class='textbld' bgcolor="#1B476E"><font color="#FFFFFF">ADD A NEW TRAFFIC SITE</font></td>

				</tr>
       <form name="step1frm" action="traffic_site_new.php" method="post">
...
<? if( isset($error_msg) && $error_msg !=null ){echo $error_msg;} ?>
               ...
               <tr>
                    ...
                    <td  colspan="3" align=center>
                      <input type=image  src="../images/submit55.png" >  &nbsp;&nbsp;<img src="../images/cancel55.png"   onclick="javascript:window.location='traffic_sites.php'">
...
of visitors to your sites, we suggest you add your sites multiple times.  You can add up to <? echo $traffic_sites_allowed; ?> Login Sites.
