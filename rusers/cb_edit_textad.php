...
include'../config.php';
require_login();
$title="$CONFIG->sitename Edit Text Ad";
global $ce;
$ce=$_GET['id'];
include("$CONFIG->templatedir/header.php");
$page_content="<div class=text id=heading  align=center><b>Edit CB Text Ad</b><br><br></div>";
if(!$_POST){
...
<table align=\"center\" class=\"shadowed\" class=text width-\"100%\" bgcolor=\"#d8ecff\" valign=top cellpadding=\"5\" cellspacing=\"0\" border=\"1\" style=\"border-collapse: collapse\" id=\"AutoNumber1\" bordercolor=\"#999999\">
 ...
 <tr>
    ...
    <td><input type=text class=input name=btitle size=40 maxlength=25 value=\"". stripslashes( $q1['title'] ) ."\"></td>
  </tr>


 <tr>
    <td align=\"left\" valign=\"middle\"> Text Ad Description:</td>
    <td>   <input type=text class=input name=btext size=40 maxlength=180 value=\"". stripslashes( $q1['text']) ."\"> </td>
  </tr>
  ...
  <tr>
    <td colspan=\"2\">
	<input type=hidden name=ce value='$ce'><input type=submit class=button value=\"Edit CB Text Ad \"><br><br></td>
  </tr>
</table><br><br> </td></tr></table></form>";}
}//if
else
{$ce=$_POST['ce'];
$s=db_query("update cb_textads set     title='".addslashes(htmlspecialchars($_POST['btitle']))."' ,       text='".addslashes(htmlspecialchars($_POST['btext']))."',url='".$_POST['burl']."' ,urlto='".$_POST['btourl']."' where id='$ce'");
