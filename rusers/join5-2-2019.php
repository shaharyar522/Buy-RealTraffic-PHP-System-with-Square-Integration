...
if(($CONFIG->allowsolidtrustpay)&&(validate_email($rss123["paypal"]))){$page_content.=add_solidtrustpay($CONFIG->sponsorfee+$CONFIG->adminfee,$rss123["solidtrustpay"],"3"   ,$rss123["username"],$rss123["email"],$rss123["firstname"],$rss123["lastname"],$_SESSION["user"]["username"]  );}
                                                               }else{ 
             if($rss["username"] !='wespac59' )
{      

 if( $fetch_sponsor["membership_status"]=='Free Member'){ $CONFIG->sponsorfee = $CONFIG->sponsorfee_free;   }                                                     
   if(($CONFIG->allowsolidtrustpay)&&(validate_email($fetch_sponsor["paypal"]))){$page_content.=add_solidtrustpay_new($CONFIG->sponsorfee,$fetch_sponsor["solidtrustpay"],"2",$fetch_sponsor["username"],$fetch_sponsor["email"],$fetch_sponsor["firstname"],$fetch_sponsor["lastname"] ,$_SESSION["user"]["username"]    );}
}            else  {            if( $fetch_sponsor["membership_status"]=='Free Member'){   $CONFIG->adminfee = $CONFIG->adminfee_free;   }  

	if(($CONFIG->allowsolidtrustpay)&&(validate_email($rss["paypal"]))    ){  $page_content.=add_solidtrustpay($CONFIG->adminfee,$rss["solidtrustpay"] ,"2",$fetch_sponsor["username"],$fetch_sponsor["email"],$fetch_sponsor["firstname"],$rss["lastname"],$_SESSION["user"]["username"]);  } 
             // unset($_SESSION["sponsor_satt"]);
