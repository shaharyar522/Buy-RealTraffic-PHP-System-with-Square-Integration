<?
include'../config.php';
$title="Join now to $CONFIG->sitename";
include("$CONFIG->templatedir/header.php");
$firstname =$_SESSION["user"]["firstname"];
$lastname =$_SESSION["user"]["lastname"];

$username =$_SESSION["user"]["username"];
$password =$_SESSION["user"]["password"];
$q= db_query("SELECT * FROM users	WHERE username = '$username' AND password = '$password' and id>'1'");

$user= db_fetch_array($q);

$ss_sponsor = $user["sponsor"];
$q_spp= db_query("SELECT * FROM users	WHERE username = '$ss_sponsor' and id>'1'	");
$fetch_sponsor= db_fetch_array($q_spp);
$fetch_sponsor["membership_status"] ?? '';
$fetch_sponsor["venmo_username"] ?? '';




$admin_q= db_query("SELECT * FROM users	WHERE  id='1'");
$user_admin= db_fetch_array($admin_q);


$page_content="
<div align=center class=text>";
$page_content=html_entity_decode(stripslashes($page_content));


                           if( $user["sponsor"] =='wespac59' ){ 
$total_amount = $CONFIG->sponsorfee  + $CONFIG->adminfee;

$total_amount =  number_format($total_amount,2);

$page_content.="


<table align=\"center\" class=\"shadowed\" valign=top width=\"100%\" height=50 bgcolor=#d8ecff cellpadding=\"25\" cellspacing=\"0\" border=\"0\" 
style=\"border-collapse: collapse\" id=\"AutoNumber1\" bordercolor=#999999> 
<tr><td>";


$page_content.="<table align=\"center\" class=\"shadowed\" class=text width-\"100%\" bgcolor=\"#d8ecff\" valign=top cellpadding=\"5\" cellspacing=\"0\" border=\"0\" 
style=\"border-collapse: collapse\" id=\"AutoNumber1\" bordercolor=\"#999999\">

<tr><td>";

$page_content.="<div align='center'><br>";
$page_content.=   html_entity_decode(grab_content("joinp")  );
$page_content.="</div>";


$page_content.="<div align=left>
<ul> 
 <li>Since you do not have a sponsor, you will pay the admin the sponsor fee of $$CONFIG->sponsorfee and the admin fee of $$CONFIG->adminfee.</li><br> ";

$page_content.="<li>You will make one single payment in the amount of $$total_amount.  </li>
<br>


<br>

</ul>

<br><br>

";
}
else{
                 
         if($fetch_sponsor["membership_status"] == 'Free Member')
         { 

   $CONFIG->sponsorfee =$CONFIG->sponsorfee_free;   $CONFIG->adminfee = $CONFIG->adminfee_free;

         }
          $usssr1 = $fetch_sponsor["firstname"] ." " .  $fetch_sponsor["lastname"] ;
          $usssr2 =  " ( ".$user["sponsor"]  . " / " .$fetch_sponsor["membership_status"]." / " .$fetch_sponsor["venmo_username"]." )" ;
$usssr3 =  " ( ".$user["username"]  . ")" ;
      
$usssr4 =  " ".$fetch_sponsor["membership_status"]  . "" ;
$usssr5 =  "To Mike Borders (Admin) " ;
$page_content.="</div>


<table align=\"center\" class=\"shadowed\" valign=top width=\"100%\" height=50 bgcolor=#d8ecff cellpadding=\"25\" cellspacing=\"0\" border=\"0\" 
style=\"border-collapse: collapse\" id=\"AutoNumber1\" bordercolor=#999999> 

<tr><td>

<table align=\"center\" class=\"shadowed\" class=text width-\"100%\" bgcolor=\"#d8ecff\" valign=top cellpadding=\"5\" cellspacing=\"0\" border=\"1\" 
style=\"border-collapse: collapse\" id=\"AutoNumber1\" bordercolor=\"#999999\">

   <tr><td><div align=left>


<br>";


if($fetch_sponsor["membership_status"]  == 'Free Member'  OR  $fetch_sponsor["membership_status"]  == 'Pro Member')
{
    


//24-2-2023------------------------------


    
                $satya=mysqli_query($GLOBALS["___mysqli_ston"], "select * from users  where username='".$ss_sponsor."'");  // 26-2-2023
                //$satya=mysqli_query($GLOBALS["___mysqli_ston"], "select * from users  where username='".$username."'");
                
                
                //$satya=mysqli_query($GLOBALS["___mysqli_ston"], "select * from users  where username='".$ss_sponsor."' and  username !='wespac59'");
	            $rss = mysqli_fetch_array($satya);
	            $to =$rss["email"];
	            $subject = $rss["firstname"].", Did You Receive Your Sponsor Fee?";
	
	
$q_admin= db_query("SELECT * FROM users	WHERE username = 'wespac59'  and id='1'");
$user_admin= db_fetch_array($q_admin);
$venmo_admin = $user_admin["venmo_username"];



	
	            $message = "
	<html>
	<head>
	<title>EMAIL TO THE BUYER'S SPONSOR</title>
	</head>
	<body>
	
	Hi ";
	$message .= $rss["firstname"];
	$message .=",<br><br>
	Mike here.  Just a short note to see if you received your sponsor fee from
<br>
your referral below.  Please reply to this email with a Yes or No.  If you have questions, 
<br>
please write us back.  Below are your referral's details.

<br><br>

Your Referral Name: ";
	$message .= $firstname;
		$message .= " ";
	$message .= $lastname ;

$message .="
<br><br>

Your Referral EZ-Money Username: ";

	$message .= $username ; 
$message .="

<br><br>

Your Referral Email Address: ";

	$message .= $_SESSION["user"]["email"] ; 
$message .="
<br><br>

Your Referral Venmo Username: ";

	$message .= $_SESSION["user"]["venmo_username"] ;
//	$message .= $venmo_admin;
$message .="

<br><br>

Sponsor Fee Amount: $";
	$message .= $CONFIG->sponsorfee;
$message .=" 
<br><br>
Sincerely,
<br><br>
Mike
<br>
<a href=http://www.ez-moneymaker.com>http://www.ez-moneymaker.com</a>   ";
               
               
                
               $mailHeader = "From: ".$CONFIG->support. "<".$CONFIG->support. ">". "\r\n";
    	       $mailHeader .= "X-Sender: $CONFIG->support "."\r\n";
    	       $mailHeader .= "Content-type:text/html;charset=UTF-8" . "\r\n";
    	       $mailHeader .= "Return-Path: $CONFIG->support "."\r\n";
    	       $mailHeader .= "Error-To: $CONFIG->support "."\r\n";
    	       $mailHeader .= "X-Mailer: {$_SERVER['SERVER_NAME']} "."\r\n";
    	     
    	      $sponsor_email =$to;
    	    
            mail($sponsor_email,$subject,$message,$mailHeader);  


//24-2-2023------------------------------


    
$page_content.= "<div align=center>" ;
$page_content.= html_entity_decode(grab_content('joinp'))  ;
$page_content.= "</div>" ;

}

$page_content.="

<br>
</div> 

<ul><b>Last Step To Upgrade</b></ul>
<ul><li>Pay<b> $$CONFIG->adminfee</b> ".$_SESSION["sponsor"]["firstname-----"]
."  ".$_SESSION["sponsor"]["lastname-----"].      $_SESSION["user"]["sponsor---"].


   "</font>";$page_content.=$usssr5;$page_content.="</li><br>";


$page_content.="
<br><br>
Please Follow The Steps Below:

<br><br>

1)  In the Amount Field, Enter The Admin Fee Above.
<br><br>
2)  Copy The Admin's Venmo Username <b>";


//$page_content.=$_SESSION["user"]["venmo_username"];

$page_content.= $user_admin["venmo_username"];



$page_content.="</b> and Paste It Into the \"To\" Field.
<br><br>
3)  Copy This Note <b>Admin Fee From User:  $username ($firstname $lastname)</b> Paste It Into The \"Note\" Field And Click The Pay Button.  Once You Have
Paid The Admin Fee, Come Back To This Page And Click The Click Here To Continue Button Below.


</ul>

<br>
<br>

<br>
<center>
<a href =/rusers/thank_you.php><img src =/images/continue.png></a>
</center>
<br><br>

";
}


$page_content.="

<script>
function validateForm() {
    var x = document.forms[\"myForm\"][\"ts\"].checked;
    if (!x) {
        alert(\"You must agree with our Terms and Conditions !\");
        return false;
    }
}
</script>


<script>
function validare()
{  if(document.forms[0].ts.checked){document.forms[0].submit();}
else{alert('You must agree with our Terms and Conditions !');}
}
var popUpWin=0;
function popUpWindow(URLStr, left,top, width, height)
{
  if(popUpWin)
  {
    if(!popUpWin.closed) popUpWin.close();
  }
  popUpWin = open(URLStr, 'popUpWin', 'toolbar=no,location=no,directories=no,status=no,menub ar=no,scrollbars=yes,resizable=no,copyhistory=yes,width='+width+',height='+height+',left='+left+', top='+top+',screenX='+left+',screenY='+top+'');
}
</script>
<form name=myForm method=post action=join.php";


if( !( isset( $_SESSION["sponsor_satt"]["username"]  ) && $_SESSION["sponsor_satt"]["username"] != null ))
$page_content.=" target=_blank ";


$page_content.=">

 <!--<input name=\"ts\" type=\"checkbox\" value=\"ts\" class=\"input\">
I agree with ".$CONFIG->sitename." <a href=\"javascript:popUpWindow('http://ez-moneymaker.com/terms.php', 100, 100, 300, 399);\" class=link1 >Terms and Conditions</a><br>-->
 <!-- <div align=center><input type=button class=button value=\"Upgrade To Pro\" onclick=\"validare();\"></div> -->


                        <!-- satyajeet -->
";


    if( $user["sponsor"] =='wespac59' )
{

$page_content.="<br><div align=center>
<input type=\"hidden\" name=\"total_amount\" value=\"$total_amount\">
<input  type=submit class=button value=\"Click Here To Pay The Admin\"  name=\"paytheadmin_xxx\"  onclick=\"return validateForm()\">
<br><br>


</div>";


 }else{
 $page_content.="<br>";


if($fetch_sponsor["membership_status"]  != 'Free Member'  && $fetch_sponsor["membership_status"]  != 'Pro Member')
 $page_content.="<div align=center> <input type=button class=button value=\"Click Here To Pay The Admin\" onclick=\"validare();\"></div> ";

$page_content.="
<div align=center>

</div>";
   }


$page_content.="
                        <!-- satyajeet -->

</form></fieldset></div></td></tr></table> </td></tr></table>           ";
include("$CONFIG->templatedir/content.php");
include("$CONFIG->templatedir/footer.php");
?>
