...
//8-2-2023
/*
            if( isset($_POST["paytheadmin_xxx"]) && $_POST["paytheadmin_xxx"]!= null ) {
              
               
                $satya=mysqli_query($GLOBALS["___mysqli_ston"], "select * from users  where username='".$ss_sponsor."'");
                //$satya=mysqli_query($GLOBALS["___mysqli_ston"], "select * from users  where username='".$ss_sponsor."' and  username !='wespac59'");
...
Your Referral Venmo Username: ";
	...
	$message .= $rss["venmo_username"] ;
$message .="

<br><br>

Sponsor Fee Amount: $";
	$message .= $_POST["total_amount"];
$message .=" 
<br><br>
Sincerely,
<br><br>
...
$rss = mysqli_fetch_array($satya);
$page_content="
<div width=300 style='    box-shadow: rgba(0,0,0,0.5) 0px 0px 24px;
    background: #BED4EB;
    -webkit-box-shadow: rgba(0,0,0,0.5) 0px 0px 24px;
    -moz-box-shadow: rgba(0,0,0,0.5) 0px 0px 24px;
    -ms-box-shadow: rgba(0,0,0,0.5) 0px 0px 24px;
    -o-box-shadow: rgba(0,0,0,0.5) 0px 0px 24px;
    filter: drop-shadow(0px 8px 8px rgba(0, 0, 0, 0.2));
    border-radius: 12px;' align=center class=text><br><br>
<font color=red>".$rss["firstname"]
."  ".$rss["lastname"]."</font>";

/* 23 apr2018
$page_content="
<div align=center class=text>
Your sponsor is :<font color=red>".$_SESSION["sponsor_satt"]["firstname"]
...
$page_content.="<br>
 ...
 if( $fetch_sponsor["membership_status"] == 'Free Member'){   $CONFIG->adminfee = $CONFIG->adminfee_free;   }  		
		...
		$page_content="
		<div style='    box-shadow: rgba(0,0,0,0.5) 0px 0px 24px;
    background: #BED4EB;
    -webkit-box-shadow: rgba(0,0,0,0.5) 0px 0px 24px;
    -moz-box-shadow: rgba(0,0,0,0.5) 0px 0px 24px;
    -ms-box-shadow: rgba(0,0,0,0.5) 0px 0px 24px;
    -o-box-shadow: rgba(0,0,0,0.5) 0px 0px 24px;
    filter: drop-shadow(0px 8px 8px rgba(0, 0, 0, 0.2));
    border-radius: 12px;' align=center class=text >
		<br><br>//Your sponsor is :<font color=red>".($_SESSION["sponsor"]["firstname-----"] ?? '')
		."  ".($_SESSION["sponsor"]["lastname-----"] ?? '').           $user["sponsor---"]. $usssr1 .  "</font>"; $page_content.=$usssr2;
		
		
		$page_content.="
<br>
		Please Pay Your Sponsor $";
...
$rss123 = mysqli_fetch_array($satya123);            
    ...
    filter: drop-shadow(0px 8px 8px rgba(0, 0, 0, 0.2));
    border-radius: 12px;' align=center class=text><br><br>
		<font color=red>".($_SESSION["sponsor"]["firstname----"] ?? '')
		."  ".($_SESSION["sponsor"]["lastname-----"] ?? '').        $rss123["firstname"]." ".$rss123["lastname"] .       "</font>";
		
		
$aamt=$CONFIG->sponsorfee+$CONFIG->adminfee;		
$aamt =  number_format($aamt,2);
...
if(($CONFIG->allowsolidtrustpay)&&(validate_email($rss123["email"]))){$page_content.=add_solidtrustpay($CONFIG->sponsorfee+$CONFIG->adminfee,$rss123["solidtrustpay"],"3"   ,$rss123["username"],$rss123["email"],$rss123["firstname"],$rss123["lastname"],$_SESSION["user"]["username"] ,$user["firstname"],$user["lastname"] , $user["solidtrustpay"]   );}
                                                               }else{  
             if($rss["username"] !='wespac59' )
{      

 if( $fetch_sponsor["membership_status"]=='Free Member'){ $CONFIG->sponsorfee = $CONFIG->sponsorfee_free;   }                                                     
   if(($CONFIG->allowsolidtrustpay)&&(validate_email($fetch_sponsor["email"]))){  $page_content.=add_solidtrustpay_new($CONFIG->sponsorfee,$fetch_sponsor["solidtrustpay"],"2",$fetch_sponsor["username"],$fetch_sponsor["email"],$fetch_sponsor["firstname"],$fetch_sponsor["lastname"] ,$_SESSION["user"]["username"] ,$user["firstname"],$user["lastname"] , $user["solidtrustpay"]  );}
}            else  {            if( $fetch_sponsor["membership_status"]=='Free Member'){   $CONFIG->adminfee = $CONFIG->adminfee_free;   }  

	if(($CONFIG->allowsolidtrustpay)&&(validate_email($rss["email"]))    ){  $page_content.=add_solidtrustpay($CONFIG->adminfee,$rss["solidtrustpay"] ,"2",$fetch_sponsor["username"],$fetch_sponsor["email"],$fetch_sponsor["firstname"],$rss["lastname"],$_SESSION["user"]["username"] ,$user["firstname"],$user["lastname"] , $user["solidtrustpay"] );  } 
             // unset($_SESSION["sponsor_satt"]);
...
{      
 ...
 if( $fetch_sponsor["membership_status"]=='Free Member'){ $CONFIG->sponsorfee = $CONFIG->sponsorfee_free;   }                                                     
   if(($CONFIG->allowpaypal)&&(validate_email($fetch_sponsor["paypal"]))){$page_content.=add_paypal_new($CONFIG->sponsorfee,$fetch_sponsor["paypal"],"2",$fetch_sponsor["username"],$fetch_sponsor["email"],$fetch_sponsor["firstname"],$fetch_sponsor["lastname"] ,$_SESSION["user"]["username"]    );}
}            else  {            if( $fetch_sponsor["membership_status"]=='Free Member'){   $CONFIG->adminfee = $CONFIG->adminfee_free;   }  
	//$q1=db_fetch_array(db_query("select username , paypal ,stormpay,egold,libertyreserve ,assuredpay from users where id='1'"));
...
if(($CONFIG->allowsolidtrustpay)&&(!empty($_SESSION["sponsor"]["solidtrustpay"]))){

  //satt15jan2019 $page_content.=add_solidtrustpay($CONFIG->sponsorfee,$_SESSION["sponsor"]["solidtrustpay"],"1");
}
