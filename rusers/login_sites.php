...
//$page_content=read_template("$CONFIG->templatedir/login_sites.php");
$mode = $_GET["mode"] ?? '';
if(isset($_GET["mode"])){  $mode = $_GET["mode"]; }
	...
	if($mode == "delete") {	
	       $sx_pts = login_site_balance($sid);    
       	   //   $sql_xyz = "SELECT login_credits FROM url_unassigned_login_credits WHERE user_id =".$_SESSION["user"]["id"]; 
echo $sql_xyz = "SELECT login_credits FROM url_unassigned_login_credits WHERE user_id ='".$_SESSION["user"]["username"]."'"; 
           ...
           //updated pts
    	//   $sql_reqx = "UPDATE  url_unassigned_login_credits SET login_credits = '$updates_pts' WHERE user_id = ".$_SESSION["user"]["id"];  
 echo   $sql_reqx = "UPDATE  url_unassigned_login_credits SET login_credits = '$updates_pts' WHERE user_id = '".$_SESSION["user"]["username"]."'";

 	       $res_reqx = mysqli_query($GLOBALS["___mysqli_ston"], $sql_reqx)or die(mysqli_error($GLOBALS["___mysqli_ston"]).$sql_reqx);

           //remove pts
	   	   $sql = "DELETE FROM url_login_sites  WHERE sid = '$sid' LIMIT 1";
 		   $res = mysqli_query($GLOBALS["___mysqli_ston"], $sql)or die(mysqli_error($GLOBALS["___mysqli_ston"]).$sql);
 
           //exchange pts
	   	   $sql_cc = "DELETE FROM url_login_clicks  WHERE site_id = '$sid' LIMIT 1";
        ...
        if($mode == "live") {		// Active Site
		   $sql = "UPDATE  url_login_sites SET status = 'L'  WHERE sid = '$sid'";
 		   $res = mysqli_query($GLOBALS["___mysqli_ston"], $sql)or die(mysqli_error($GLOBALS["___mysqli_ston"]).$sql);
	...
	if($mode == "hide") {		// Active Site
		   $sql = "UPDATE  url_login_sites SET status = 'H'  WHERE sid = '$sid'";
 		   $res = mysqli_query($GLOBALS["___mysqli_ston"], $sql)or die(mysqli_error($GLOBALS["___mysqli_ston"]).$sql);
	...
	if($mode == "reset") {	
		   ...
		   $sql = "UPDATE  url_login_clicks SET visitors_received = '0'  WHERE site_id = '$sid'";
 		   $res = mysqli_query($GLOBALS["___mysqli_ston"], $sql)or die(mysqli_error($GLOBALS["___mysqli_ston"]).$sql);
    ...
    // $unassigned_login_credits_query = "SELECT * FROM url_unassigned_login_credits WHERE user_id = ".$_SESSION["user"]["id"];
     $unassigned_login_credits_query = "SELECT * FROM url_unassigned_login_credits WHERE user_id ='".$_SESSION["user"]["username"]."'";
...
function login_site_meet($sid){
    ...
    return $row_count_info[0];   
      ...
      $max_sites_allowed = $row_max['login_sites_allowed']; 
...
<tr><td align="center">';
 ...
 $page_content .='
<br>
<center><h2>Your Login Sites & Stats</h2></center>
<br>
<center><b>Unassigned login credits:&nbsp;&nbsp;<font color=green>';
 $page_content .=  $login_credits;
  $page_content .='</font> </b></center>
<br>
<a href="login_site_share.php"><font color=#000000><img src="../images/assign-login-credits.png" ></font></a>
<br><br>
...
//  $queryy = "select * from url_login_sites WHERE user_id=55";
    ...
    $queryy = "select * from url_login_sites WHERE user_id='".$_SESSION["user"]["username"]."' order by sid desc";
...
if($r["status"] == 'A'){  $sss = 'SUSPENDED'; }
 ...
 if($r["status"] == 'H'){
  ...
  $page_content .="<a href='login_sites.php?mode=live&sid=";
  $page_content .= $r["sid"];
  ...
  $page_content .="<a href='login_sites.php?mode=hide&sid=";
  $page_content .= $r["sid"];
...
// $_SESSION["user"]["id"]
  $page_content .="<a href='login_site_edit.php?site_id=";
  $page_content .= $r["sid"];
  $page_content .=  "' title='Click To Edit Site'  >  <img src='../images/edit.png' border=0> </a>"; 
 
  $page_content .="<a href='login_sites.php?mode=delete&sid=";
  $page_content .= $r["sid"];
  $page_content .=  "'  onclick=\"return confirm('Are you sure you want to delete this site?');\"   title='If you DELETE a site with login credits assigned, you will not lose those credits.  Your credits will be returned to your UNASSIGNED LOGIN account above.'  >  
  <img src='../images/delete.png' border=0> </a>"; 
  
  $page_content .="<a href='login_sites.php?mode=reset&sid=";
  $page_content .= $r["sid"];
...
$page_content .='<br /><br />
        <p align=center>
           <a href=\'login_site_new.php\' onClick="return sitesadd({$num_vv},{$max_sites_allowed});" ><img src="../images/add-login-site.png" ></a>
           <br />
...
<blockquote>
<font color=red>
Note:  Please <b>DO NOT</b> add a login site unless you have Login Credits in your "Unassigned Login Credits" account above or Login Credits assigned to your current sites.
</font>
</blockquote>
</td></tr></table>';
...
//---------------------satyajeet 7-7-2018
    ...
    if(isset($_REQUEST['mode']) && $_REQUEST['mode'] == 'update_share'){
      $sel_count = "SELECT site_id, points as SUMMER ,user_id FROM url_login_clicks WHERE user_id = '".$_SESSION["user"]["username"]."' AND site_id != '{$_REQUEST['site_id']}' GROUP BY site_id  ORDER BY site_id ASC "; 
       $res_count = mysqli_query($GLOBALS["___mysqli_ston"], $sel_count) or die(mysqli_error($GLOBALS["___mysqli_ston"]).$sel_count);
...
//       $smarty->assign('contentx',$contentx);
   ...
   if(isset($_REQUEST['points']) && $_REQUEST['points'] == ''){
     ...
     if(isset($_REQUEST['mode']) && ($_REQUEST['mode'] == 'update_share') && ($err_msg_share== "") && ($_REQUEST['points'] != '')  ) {
       ...
       $sel_old = "UPDATE url_login_clicks SET points  = '$Curr_from_site_points' WHERE site_id = '$site_id'";
       $res_old = mysqli_query($GLOBALS["___mysqli_ston"], $sel_old) or die(mysqli_error($GLOBALS["___mysqli_ston"]).$sel_old);
       $sel_new = "UPDATE url_login_clicks SET points  = '$Curr_to_site_points' WHERE site_id = '$to_site_id'";
       $res_new = mysqli_query($GLOBALS["___mysqli_ston"], $sel_new) or die(mysqli_error($GLOBALS["___mysqli_ston"]).$sel_new);
      // header("Location: http://www.ez-moneymaker.com/rusers/login_sites.php?E=1&F=$from_site_id&T=$to_site_id#MOVE");

redirect("$CONFIG->siteurl/rusers/login_sites.php?E=1&F=$from_site_id&T=$to_site_id#MOVE","",0);
       die();
    }
...
function get_site_name($sid){
    ...
    $sel_count = "SELECT site_id, points as SUMMER ,user_id  FROM url_login_clicks WHERE user_id = '".$_SESSION["user"]["username"]."'   GROUP BY site_id  ORDER BY site_id ASC "; 
     $res_count = mysqli_query($GLOBALS["___mysqli_ston"], $sel_count) or die(mysqli_error($GLOBALS["___mysqli_ston"]).$sel_count);
     if (! isset($content)) {
        $content = '';
     }
...
$page_content .='<table border="0" width="790" cellspacing="0" cellpadding="0" align="center" style=" border: 1px solid #808080;font-size: 14px;"><a name="MOVE"></a>
           <form method="post" name="ThisForm" action="login_sites.php?#MOVE">';
                if(isset($err_msg_share)){
          ...
          <td colspan="3" align="right" class="textbld" bgcolor="#1B476E"><font color="#FFFFFF">MOVE CREDITS</font></td>
               ...
               <tr>
                   ...
                   <input type=image src="/images/move-login-credits.png" border=0  >
 <br><br>
