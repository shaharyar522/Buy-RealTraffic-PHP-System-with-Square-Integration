...
<br>Your sponsor is :<font color=red>".$rss["firstname"]
."  ".$rss["lastname"]."</font>";
...
$q1=db_fetch_array(db_query("select username , paypal ,stormpay,egold,libertyreserve ,assuredpay from users where id='1'"));
if(($CONFIG->allowpaypal)&&(validate_email($_SESSION["sponsor"]["paypal"]))){$page_content.=add_paypal($CONFIG->sponsorfee+$CONFIG->adminfee,$q1["paypal"],"3"   ,$_SESSION["sponsor"]["username"],$_SESSION["sponsor"]["email"],$rss["firstname"],$rss["lastname"]  );}
                                                               }else{
             if($rss["username"] !='wespac59' )
{                                                              
   if(($CONFIG->allowpaypal)&&(validate_email($_SESSION["sponsor"]["paypal"]))){$page_content.=add_paypal_new($CONFIG->sponsorfee,$_SESSION["sponsor"]["paypal"],"2",$_SESSION["sponsor"]["username"],$_SESSION["sponsor"]["email"],$_SESSION["sponsor"]["firstname"],$_SESSION["sponsor"]["lastname"]);}
}            else  {
	//$q1=db_fetch_array(db_query("select username , paypal ,stormpay,egold,libertyreserve ,assuredpay from users where id='1'"));
	if(($CONFIG->allowpaypal)&&(validate_email($rss["paypal"]))    ){$page_content.=add_paypal($CONFIG->adminfee,$rss["paypal"] ,"2",$_SESSION["sponsor"]["username"],$_SESSION["sponsor"]["email"],$_SESSION["sponsor"]["firstname"],$rss["lastname"]);  } 
             // unset($_SESSION["sponsor_satt"]);
